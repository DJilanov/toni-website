    ...
    .controller('ProductCtrl', ['$scope', '$location', '$http', 'sharingSvc', '$rootScope',
        function($scope, $location, $http, sharingSvc, $rootScope) {
            ...
            $scope.callback = function(products, categories) {
                ...
                // if we come from the select by make we wont have category in the link. We will have make
                if ($scope.category === null) {
                    for (var categoryCounter = 0; categoryCounter < products.length; categoryCounter++) {
                        for (var productCounter = 0; productCounter < products[categoryCounter].length; productCounter++) {
                            if (products[categoryCounter][productCounter].make !== undefined) {
                                if (products[categoryCounter][productCounter].make.toUpperCase() === $scope.getLocation()) {
